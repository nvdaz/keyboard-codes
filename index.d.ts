...
export type AlphanumericWritingSystemKeyCode =
...
export type AlphanumericFunctional1KeyCode =
  // {Alt}, {Option} or {⌥}.
  | 'AltLeft'
...
export type AlphanumericFunctional2KeyCode =
  // Japanese: {変換} ({henkan})
  | 'Convert'
...
export type ControlpadKeyCode =
  // {⌦}. The forward delete key. Note that on Apple keyboards, the key labelled {Delete} on the main part of the keyboard should be encoded as
  | 'Delete'
  // {Page Down}, {End} or {↘}
  | 'End'
...
export type NumpadKeyCode =
  ...
  | 'NumpadBackspace'
  // {C} or {AC} (All Clear). Also for use with [=numpads=] that have a {Clear} key that is separate from the {NumLock} key. On the Mac, the [=numpad=] {Clear} key should always be encoded as
  | 'NumpadClear'
  // {CE} (Clear Entry)
  | 'NumpadClearEntry'
...
export type LegacyModifierKeyCode = 'Hyper' | 'Super' | 'Turbo';
/** List of code values for legacy process control keys. */
export type LegacyProcessKeyCode = 'Abort' | 'Resume' | 'Suspend';
/** List of code values for legacy editing keys. */
export type LegacyEditingKeyCode =
...
export type SpecialKeyCode =
  ...
  'Unidentified';
export type KeyCode =
  | AlphanumericWritingSystemKeyCode
  | AlphanumericFunctional1KeyCode
  | AlphanumericFunctional2KeyCode
  | ControlpadKeyCode
  | ArrowpadKeyCode
  | NumpadKeyCode
  | FunctionKeyCode
  | MediaKeyCode
  | LegacyModifierKeyCode
  | LegacyProcessKeyCode
  | LegacyEditingKeyCode
